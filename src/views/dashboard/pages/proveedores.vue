        ...
        >
          <template v-slot:top>
            <v-toolbar
              flat
            >
              <v-toolbar-title>Proveedores actuales de la empresa</v-toolbar-title>
              <v-card-title>
                <v-text-field
                  v-model="search"
...
<script>
  ...
  export default {
    ...
    methods: {
      agregarProveedor () {
        this.$router.push({ name: 'camposProveedor', params: { CodProv: '-7' } })
      },
      editarProveedor (codProvParams) {
        console.log(codProvParams)
        this.$router.push({ name: 'camposProveedor', params: { CodProv: codProvParams.codProv } })
      },
      getDataProveedores () {
        ...
        axios.get(path).then((response) => {
          ...
          for (var i = 0; i < response.data.length; i++) {
            diccion = {
              codProv: response.data[i].codProve,
              nameempresa: response.data[i].empresa,
              nametalefono: response.data[i].telefono,
              nameemail: response.data[i].email,
              namecontacto: response.data[i].contacto,
              namenotageneral: response.data[i].nota_general,
      ...
      deleteItem (item) {
        swal({
          ...
          .then((willDelete) => {
            if (willDelete) {
              var path = axios.defaults.baseURL + '/api/proveedores/proveedores/' + item.codProv + '/'
